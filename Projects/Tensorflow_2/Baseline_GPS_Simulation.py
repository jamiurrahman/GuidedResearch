...
for i in range(loopRange):
    ...
    if(currentSumOfTime >= gpsTimeToUpdate):
        ...
        v_t_plus_1.append((v_t_plus_1[i] * dumping_factor_for_velocity) + (original_acc1_x[i] * delta_t[i]))
    else:
        ...
        s_t_plus_1.append(s_t_plus_1[i] + (v_t_plus_1[i] * delta_t[i]) + (.5 * original_acc1_x[i] * (delta_t[i]**2)))
